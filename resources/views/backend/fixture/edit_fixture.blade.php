            ...
            <div class="intro-y box p-5">
                <div class="mt-3">
                    ...
                    <select data-placeholder="Select Season" class="tom-select w-full" id="season" name="season">
                        ...
                        <option value="{{($season->id)}}" {{$season->id  == $fixture->season_id ? 'selected' : '' }} >{{($season->season_name)}}</option>
                        @endforeach
                    </select>
                ...
                <div class="mt-3">
                    ...
                    <select data-placeholder="Select Team" class="tom-select w-full" id="first_team" name="first_team">
                        ...
                        <option value="{{($team->id)}}" {{$team->id  == $fixture->first_team ? 'selected' : '' }} >{{($team->name)}}</option>
                        @endforeach
                    </select>
                ...
                <div class="mt-3">
                    ...
                    <select data-placeholder="Select your favorite actors" class="tom-select w-full" id="second_team" name="second_team">
                        <option value="" >--select--</option>
                        @foreach ($teams as $team)
                        <option value="{{($team->id)}}" {{$team->id == $fixture->second_team ? 'selected' : '' }} >{{($team->name)}}</option>
                        @endforeach
                    </select>
                ...
                <div class="mt-3">
                    ...
                    <div class="sm:grid grid-cols-2 gap-2">
                        <div class="input-group">
                            ...
                            <input type="date" class="form-control" placeholder="Date" aria-describedby="date" name="date" value="{{$fixture->date}}">
                        </div>

                        <div class="input-group mt-2 sm:mt-0">
                            <div id="time" class="input-group-text">Time</div>
                            <input type="time" class="form-control" placeholder="Time" aria-describedby="time" name="time" value="{{$fixture->time}}">
                            <select  class="form-select w-full" id="time_zone" name="time_zone">
                                <option value="am" {{'am'  == $fixture->time_zone ? 'selected' : '' }}> AM</option>
                                <option value="pm" {{'pm'  == $fixture->time_zone ? 'selected' : '' }} >PM </option>
                            </select>
                        </div>
