...
@section('subcontent')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Edit Team Result</h2>
                ...
                <div class="intro-y box p-5">
                    ...
                    <div class="mt-3">
                        ...
                        <select data-placeholder="Select Team" class="tom-select w-full" id="team_one" name="team_one">
                            ...
                            @foreach ($teams as $team)
                                <option value="{{ $team->id }}" {{$team->id == $team_results->team1_id ? 'selected' : ''}}>{{ $team->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('team_one')
                    ...
                    <div class="mt-3">
                        ...
                        <select data-placeholder="Select your favorite actors" class="tom-select w-full" id="team_two"
                            ...
                            <option value="">--select--</option>
                            @foreach ($teams as $team)
                                <option value="{{ $team->id }}" {{$team->id == $team_results->team2_id ? 'selected' : ''}}>{{ $team->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('team_two')
                    ...
                    <div class="mt-3">
                        ...
                        <div class="input-group">
                            <input id="team1_score" type="text" class="form-control" placeholder="team1_score"
                                aria-describedby="input-group-1" name="team1_score" value="{{$team_results->team1_score}}">
                        </div>
                        @error('team1_score')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mt-3">
                        ...
                        <div class="input-group">
                            <input id="team2_score" type="text" class="form-control" placeholder="team2_score"
                                aria-describedby="input-group-1" name="team2_score" value="{{$team_results->team2_score}}">
                        </div>
                        @error('team2_score')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                            ...
                            <div class="sm:grid grid-cols-2 gap-2">
                                <div class="input-group mt-2 sm:mt-0">
                                    <select class="form-select w-full" id="result_status" name="result_status">
                                        <option value="">--select--</option>
                                        <option value="win" {{$team_results->result_status == 'win' ? 'selected' : ''}}>Win</option>
                                        <option value="loss"  {{$team_results->result_status == 'loss' ? 'selected' : ''}}>Loss</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="sm:grid grid-cols-2 gap-2">
                                    <div class="input-group mt-2 sm:mt-0 relative">
                                        <select class="form-select w-full" id="status" name="status">
                                            <option value="">--select--</option>
                                            <option value="active" {{$team_results->status == 'active' ? 'selected' : ''}}>Active</option>
                                            <option value="Inactive" {{$team_results->status == 'inactive' ? 'selected' : ''}}>Inactive</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
